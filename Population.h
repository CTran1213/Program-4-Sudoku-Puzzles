...
//Population or its subclasses must implement constructors (that create an 
//initial, random set of Puzzles), a "cull" method (that eliminates the X% 
//least fit members of a generation), a "newGeneration" method (that produces
// a new generation with N individuals for every single individual in the old
// generation — presumably after the old generation has been culled, so that 
//the population size stays constant from one generation to the next), a 
//"bestFitness" method (that returns the fitness of the best Puzzle in the 
//current generation), and a "bestIndividual" method (that returns the Puzzle
